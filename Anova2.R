# X1 is customer type, 3 different types of costumers (1=less than a year, 2=between 1 and 
# 5 years, 3=longer than 5 years).
...
# Anova result tells us that there are differences in the mean values of the groups, but it 
# doesn't tell anything about which groups are different. We colud try a series of t.test() 
# but they don't control for family wise error rate. If we don't control
...
# We find significant differences in 1 and 2, and, 1 and 3. But this is not the way to proceed 
# when we are making many comparison of this type. We have to adjust p-values.
#Using the function aov, it permits us to do that. 
anova2=aov(X20~X1, data=hbat)

# Once we have model anova2, fitted with the function aov(), we can use Levene test to test
# constant variances. It is less sensitive to non normality, package car.
leveneTest(anova2)
summary(anova2)
