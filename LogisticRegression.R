...
# We start with an automatic procedure to find a logistic model for X23(=extent to which the respondent preceives his firm 
# would engage in partnership with HBAT 0=not, 1=yes) in terms of variables X6 to X21
...
# The next part of the output shows the coefficients, their standard errors, the z-statistic (sometimes called a Wald z-statistic),
# and the associated p-values. The logistic regression coefficients give the change in the log odds of the outcome for a one unit
...
# p-value < 0.05 supporting Model 2 (log.m), but perhaps model 1 has a clearer interpretation
